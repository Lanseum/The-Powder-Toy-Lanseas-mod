...
void Renderer::render_parts()
{
	...
	for(i = 0; i < sim->parts.active; i++) {
		if (sim->parts[i].type && sim->parts[i].type >= 0 && sim->parts[i].type < PT_NUM) {
			t = sim->parts[i].type;
			auto editLayerInactive = sim->editLayersEnabled && sim->parts[i].editLayer != sim->editLayer;
			...
			if(nx >= XRES || nx < 0 || ny >= YRES || ny < 0)
				continue;
			if(TYP(sim->photons[ny][nx]) && !(elements[t].Properties & TYPE_ENERGY) && t!=PT_STKM && t!=PT_STKM2 && t!=PT_FIGH && t!=PT_HMAN)
				continue;
...

			{
				...
				if (editLayerInactive)
				{
					colr = (colr + 110) / 2;
					colg = (colg + 130) / 2;
					colb = (colb + 140) / 2;
					cola = std::min(cola, 88);
					firea = (firea * 80) / 255;
					pixel_mode &= ~(PMODE_FLAT | PMODE_BLOB);
					pixel_mode |= PMODE_BLEND;
				}
				...
				if(pixel_mode & PSPEC_STICKMAN)
				{
					...
					else if (colorMode != COLOUR_HEAT && t != PT_HMAN)
					{
						if (cplayer->fan)
						{
					...
					if (t==PT_HMAN)
					{
						auto skin = RGB(255, 224, 160);
						auto limb = matchesFindingElement ? RGB(255, 0, 0) : RGB(246, 246, 246);
						auto cloth = limb;
						auto legRoot = Vec2<int>{ nx, ny+12 };
						auto shoulderLeft = Vec2<int>{ nx-2, ny+5 };
						auto shoulderRight = Vec2<int>{ nx+2, ny+5 };

						auto headCentre = Vec2<int>{ nx, ny };
						for (auto hy = -3; hy <= 3; hy++)
						{
							for (auto hx = -3; hx <= 3; hx++)
							{
								auto distSq = hx * hx + hy * hy;
								if (distSq <= 9 && distSq >= 5)
								{
									DrawPixel(headCentre + Vec2<int>{ hx, hy }, skin);
								}
							}
						}

						DrawLine({ nx, ny+4 }, legRoot, cloth);
						DrawLine(shoulderLeft, shoulderRight, cloth);

						if (sim->humanPose.ready)
						{
							auto leftElbow = Vec2<int>{ int(sim->humanPose.arms[0]), int(sim->humanPose.arms[1]) };
							auto leftHand = Vec2<int>{ int(sim->humanPose.arms[2]), int(sim->humanPose.arms[3]) };
							auto rightElbow = Vec2<int>{ int(sim->humanPose.arms[4]), int(sim->humanPose.arms[5]) };
							auto rightHand = Vec2<int>{ int(sim->humanPose.arms[6]), int(sim->humanPose.arms[7]) };
							DrawLine(shoulderLeft, leftElbow, limb);
							DrawLine(leftElbow, leftHand, limb);
							DrawLine(shoulderRight, rightElbow, limb);
							DrawLine(rightElbow, rightHand, limb);
						}

						DrawLine(legRoot, { int(cplayer->legs[ 0]), int(cplayer->legs[ 1]) }, limb);
						DrawLine({ int(cplayer->legs[0]), int(cplayer->legs[1]) }, { int(cplayer->legs[ 4]), int(cplayer->legs[ 5]) }, limb);
						DrawLine(legRoot, { int(cplayer->legs[ 8]), int(cplayer->legs[ 9]) }, limb);
						DrawLine({ int(cplayer->legs[8]), int(cplayer->legs[9]) }, { int(cplayer->legs[12]), int(cplayer->legs[13]) }, limb);
						DrawPixel({ int(cplayer->legs[4]), int(cplayer->legs[5]) }, limb);
						DrawPixel({ int(cplayer->legs[12]), int(cplayer->legs[13]) }, limb);
						continue;
					}
...
void Renderer::draw_human_debris()
{
	for (auto const &debris : sim->humanDebris)
	{
		auto alpha = std::clamp(debris.life < 90 ? debris.life * 3 : 235, 0, 235);
		auto colour = RGB(
			int((debris.colour >> 16) & 0xFF),
			int((debris.colour >> 8) & 0xFF),
			int(debris.colour & 0xFF)
		);
		auto toInt = [](Vec2<float> value) {
			return Vec2<int>{ int(value.X + 0.5f), int(value.Y + 0.5f) };
		};

		if (debris.kind == 1)
		{
			auto centre = Vec2<int>{ int(debris.x + 0.5f), int(debris.y + 0.5f) };
			auto radius = int(std::max(2.0f, debris.halfLength));
			auto innerRadius = std::max(1, radius - 1);
			for (auto y = -radius; y <= radius; ++y)
			{
				for (auto x = -radius; x <= radius; ++x)
				{
					...
					if (distSq <= radius * radius && distSq >= innerRadius * innerRadius)
						BlendPixel(centre + Vec2<int>{ x, y }, colour.WithAlpha(alpha));
				}
			}
			continue;
		}
		...
		auto centre = Vec2<float>{ debris.x, debris.y };
		auto start = centre - dir * debris.halfLength;
		auto end = centre + dir * debris.halfLength;
		BlendLine(toInt(start), toInt(end), colour.WithAlpha(alpha));
		BlendLine(toInt(start + normal), toInt(end + normal), colour.WithAlpha(alpha / 3));
		BlendLine(toInt(start - normal), toInt(end - normal), colour.WithAlpha(alpha / 3));
	}
}

void Renderer::draw_artillery_shells()
{
	if (sim->artilleryAiming)
	{
		auto delta = sim->artilleryAimEnd - sim->artilleryAimStart;
		auto distance = std::sqrt(float(delta.X * delta.X + delta.Y * delta.Y));
		if (distance >= 2.0f)
		{
			auto const &elements = SimulationData::CRef().elements;
			auto bombProperties = elements[PT_BOMB].Properties;
			auto shellBlocked = [this, &elements, bombProperties](int x, int y) {
				if (x < 0 || y < 0 || x >= XRES || y >= YRES)
				{
					return true;
				}

				auto wall = sim->bmap[y / CELL][x / CELL];
				if (wall)
				{
					if (wall == WL_ALLOWGAS && !(bombProperties & TYPE_GAS))
						return true;
					if (wall == WL_ALLOWENERGY && !(bombProperties & TYPE_ENERGY))
						return true;
					if (wall == WL_ALLOWLIQUID && !(bombProperties & TYPE_LIQUID))
						return true;
					if (wall == WL_ALLOWPOWDER && !(bombProperties & TYPE_PART))
						return true;
					if (wall == WL_ALLOWAIR || wall == WL_WALL || wall == WL_WALLELEC || wall == WL_DESTROYALL)
						return true;
					if (wall == WL_EWALL && !sim->emap[y / CELL][x / CELL])
						return true;
					if (wall == WL_DETECT && (bombProperties & TYPE_SOLID))
						return true;
				}
				...
				return sim->photons[y][x] != 0;
			};

			auto speed = std::clamp(distance * 0.13f, 4.0f, 28.0f);
			auto x = float(sim->artilleryAimStart.X);
			auto y = float(sim->artilleryAimStart.Y);
			auto vx = float(delta.X) / distance * speed;
			auto vy = float(delta.Y) / distance * speed;
			auto plotDistance = 0;
			for (auto life = 0; life < 220; ++life)
			{
				auto steps = std::max(1, int(std::ceil(std::max(std::abs(vx), std::abs(vy)))));
				auto stepVelocity = Vec2<float>{ vx / float(steps), vy / float(steps) };
				for (auto step = 0; step < steps; ++step)
				{
					...
					auto ix = int(x + 0.5f);
					auto iy = int(y + 0.5f);
					if (shellBlocked(ix, iy))
					{
						if (ix >= 0 && iy >= 0 && ix < XRES && iy < YRES)
						{
							BlendPixel({ ix, iy }, 0xFF4040_rgb .WithAlpha(130));
							BlendPixel({ ix - 1, iy }, 0xFF4040_rgb .WithAlpha(70));
							BlendPixel({ ix + 1, iy }, 0xFF4040_rgb .WithAlpha(70));
							BlendPixel({ ix, iy - 1 }, 0xFF4040_rgb .WithAlpha(70));
							BlendPixel({ ix, iy + 1 }, 0xFF4040_rgb .WithAlpha(70));
						}
						life = 220;
						break;
					}
					if ((plotDistance / 5) % 2 == 0)
					{
						BlendPixel({ ix, iy }, 0xFF3030_rgb .WithAlpha(62));
					}
					plotDistance++;
				}
				vx *= 0.997f;
		...
		BlendLine(sim->artilleryAimStart, sim->artilleryAimEnd, 0xFFD066_rgb .WithAlpha(100));
		BlendPixel(sim->artilleryAimStart, 0xFFFFFF_rgb .WithAlpha(170));
	}

	for (auto const &shell : sim->artilleryShells)
	{
		auto speed = std::sqrt(shell.vx * shell.vx + shell.vy * shell.vy);
		if (speed <= 0.001f)
		{
			continue;
		}

		auto dir = Vec2<float>{ shell.vx / speed, shell.vy / speed };
		auto normal = Vec2<float>{ -dir.Y, dir.X };
		auto centre = Vec2<float>{ shell.x, shell.y };
		auto nose = centre + dir * 5.0f;
		auto tail = centre - dir * 4.0f;
		auto fin = centre - dir * 2.0f;
		auto toInt = [](Vec2<float> value) {
			return Vec2<int>{ int(value.X + 0.5f), int(value.Y + 0.5f) };
		};

		BlendLine(toInt(tail + normal * 1.7f), toInt(nose), 0xF4D36A_rgb .WithAlpha(235));
		BlendLine(toInt(tail - normal * 1.7f), toInt(nose), 0x8B6F30_rgb .WithAlpha(210));
		BlendLine(toInt(tail), toInt(nose), 0xFFF5B0_rgb .WithAlpha(255));
		BlendLine(toInt(fin + normal * 3.0f), toInt(tail), 0xA64B38_rgb .WithAlpha(210));
		BlendLine(toInt(fin - normal * 3.0f), toInt(tail), 0xA64B38_rgb .WithAlpha(210));
		BlendPixel(toInt(nose), 0xFFFFFF_rgb .WithAlpha(240));
	}
}

void Renderer::draw_fpv_drone()
{
	...
	auto up = down * -1.0f;
	auto leftProp = centre - right * 3.8f + up * 3.0f;
	auto rightProp = centre + right * 3.8f + up * 3.0f;
	auto rpgMount = centre + down * 2.5f;
	auto rpgTip = centre + down * 5.8f;
	auto toInt = [](Vec2<float> value) {
		return Vec2<int>{ int(value.X + 0.5f), int(value.Y + 0.5f) };
	};
	auto drawProp = [&](Vec2<float> rotor, int phase) {
		auto spinA = right;
		auto spinB = up;
		if (((sim->currentTick + phase) / 2) & 1)
		{
			spinA = right + up;
			spinB = right - up;
			auto normalise = [](Vec2<float> value) {
				auto length = std::sqrt(value.X * value.X + value.Y * value.Y);
				return length > 0.0f ? value / length : value;
			};
			spinA = normalise(spinA);
			spinB = normalise(spinB);
		}

		BlendLine(toInt(rotor - spinA * 3.2f), toInt(rotor + spinA * 3.2f), 0xD8F6FF_rgb .WithAlpha(82));
		BlendLine(toInt(rotor - spinB * 2.4f), toInt(rotor + spinB * 2.4f), 0x7ADFFF_rgb .WithAlpha(48));
		BlendPixel(toInt(rotor), 0xFFFFFF_rgb .WithAlpha(210));
	};

	BlendLine(toInt(centre - right * 4.5f), toInt(centre + right * 4.5f), 0x8CC8FF_rgb .WithAlpha(235));
	BlendLine(toInt(centre - right * 2.8f), toInt(leftProp), 0xD0ECFF_rgb .WithAlpha(155));
	BlendLine(toInt(centre + right * 2.8f), toInt(rightProp), 0xD0ECFF_rgb .WithAlpha(155));
	BlendLine(toInt(centre), toInt(rpgMount), 0x4A6472_rgb .WithAlpha(210));
	BlendLine(toInt(rpgMount - right * 1.8f), toInt(rpgMount + right * 1.8f), 0x384850_rgb .WithAlpha(190));
	BlendLine(toInt(rpgMount - right * 1.1f), toInt(rpgTip), 0xBFA06A_rgb .WithAlpha(230));
	BlendLine(toInt(rpgMount + right * 1.1f), toInt(rpgTip), 0x7C5A32_rgb .WithAlpha(210));
	BlendPixel(toInt(rpgTip), 0xFFDE7A_rgb .WithAlpha(240));
	BlendPixel(toInt(centre), 0x9BFF80_rgb .WithAlpha(235));
	drawProp(leftProp, 0);
	drawProp(rightProp, 2);
}
