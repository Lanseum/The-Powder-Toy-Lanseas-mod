	...
	mousePosition(0, 0)
{
	...
	for (int i = 0; i < editLayerButtonCount; ++i)
	{
		auto *layerButton = new ui::Button(ui::Point(2 + i * 25, 2), ui::Point(24, 15), String::Build("L", i + 1), String::Build("Edit layer ", i + 1));
		layerButton->SetTogglable(true);
		layerButton->Appearance.BackgroundInactive = ui::Colour(0, 0, 0, 150);
		layerButton->Appearance.BackgroundHover = ui::Colour(24, 34, 42, 220);
		layerButton->Appearance.BackgroundActive = ui::Colour(52, 74, 84, 235);
		layerButton->Appearance.BorderInactive = ui::Colour(85, 100, 105, 150);
		layerButton->Appearance.BorderHover = ui::Colour(140, 220, 255, 220);
		layerButton->Appearance.BorderActive = ui::Colour(80, 220, 255, 255);
		layerButton->SetActionCallback({ [this, i] {
			c->SetEditLayer(i);
			UpdateEditLayerButtons(i);
		} });
		editLayerButtons[i] = layerButton;
		AddComponent(layerButton);
	}
	UpdateEditLayerButtons(0);
...
void GameView::NotifyPausedChanged(GameModel * sender)
{
	...
	UpdateEditLayerButtons(sender->GetEditLayer());
	ApplySimFpsLimit();
}

void GameView::UpdateEditLayerButtons(int activeLayer)
{
	for (int i = 0; i < editLayerButtonCount; ++i)
	{
		if (editLayerButtons[i])
		{
			editLayerButtons[i]->SetToggleState(i == activeLayer);
		}
	}
	UpdateEditLayerButtonFade();
}

void GameView::UpdateEditLayerButtonFade()
{
	auto layerCornerActive = currentMouse.X >= 0 && currentMouse.Y >= 0 && currentMouse.X < 112 && currentMouse.Y < 34;
	auto inactiveAlpha = layerCornerActive ? 150 : 28;
	auto hoverAlpha = layerCornerActive ? 220 : 70;
	auto activeAlpha = layerCornerActive ? 235 : 58;
	auto borderAlpha = layerCornerActive ? 150 : 36;
	auto textAlpha = layerCornerActive ? 255 : 78;

	for (auto *button : editLayerButtons)
	{
		if (!button)
		{
			continue;
		}
		button->Appearance.BackgroundInactive = ui::Colour(0, 0, 0, inactiveAlpha);
		button->Appearance.BackgroundHover = ui::Colour(24, 34, 42, hoverAlpha);
		button->Appearance.BackgroundActive = ui::Colour(52, 74, 84, activeAlpha);
		button->Appearance.BorderInactive = ui::Colour(85, 100, 105, borderAlpha);
		button->Appearance.BorderHover = ui::Colour(140, 220, 255, hoverAlpha);
		button->Appearance.BorderActive = ui::Colour(80, 220, 255, layerCornerActive ? 255 : 92);
		button->Appearance.TextInactive = ui::Colour(255, 255, 255, textAlpha);
		button->Appearance.TextHover = ui::Colour(255, 255, 255, layerCornerActive ? 255 : 120);
		button->Appearance.TextActive = ui::Colour(255, 255, 255, layerCornerActive ? 255 : 135);
	}
}
...
void GameView::RotateSave(float degrees)
{
	placeSaveTranslate = Vec2<int>::Zero;
	placeSaveRotationDegrees = degrees;
	ApplyTransformPlaceSave();
}

void GameView::PromptRotateSave()
{
	new TextPrompt("Rotate paste", "Angle in degrees", String::Build(placeSaveRotationDegrees), "45", false, { [this](String const &input) {
		auto utf8 = input.ToUtf8();
		char *end = nullptr;
		auto degrees = std::strtof(utf8.c_str(), &end);
		if (end != utf8.c_str())
		{
			RotateSave(degrees);
		}
...
void GameView::ApplyTransformPlaceSave()
{
	...
	c->TransformPlaceSave(placeSaveTransform, placeSaveRotationDegrees, { remX, remY });
}
