...
{
	if (!pauseState && paused && sim->GetEditLayersEnabled())
	{
		sim->FlattenEditLayers();
		SetInfoTip("Layers applied");
	}
...
void GameModel::CycleEditLayer(int direction)
{
	SetEditLayer(sim->GetEditLayer() + direction);
}

void GameModel::SetEditLayer(int layer)
{
	if (!paused)
	{
		SetPaused(true);
	...
	sim->SetEditLayer(layer);
	SetInfoTip(String::Build("Layer ", sim->GetEditLayer() + 1, "/", Simulation::EditLayerCount));
}

int GameModel::GetEditLayer() const
{
	return sim->GetEditLayer();
}
...
void GameModel::TransformPlaceSave(Mat2<int> transform, float rotationDegrees, Vec2<int> nudge)
{
	if (placeSave)
	{
		...
		{
			transformedPlaceSave->Transform(transform, Vec2<int>::Zero);
			transformedPlaceSave->Rotate(rotationDegrees, nudge);
		}
	}
