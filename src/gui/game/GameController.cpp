...
namespace
{
int activeArrowControl(Simulation *sim)
{
	if (sim->activeControllable == CONTROL_DRON && sim->fpvDrone.active)
		return CONTROL_DRON;
	if (sim->activeControllable == CONTROL_HMAN && sim->human.spwn)
		return CONTROL_HMAN;
	if (sim->player.spwn)
		return CONTROL_STKM;
	if (sim->human.spwn)
...
void clearArrowCommand(playerst &player, int bit)
{
	if (!bit)
		return;
	if (bit == 0x01 || bit == 0x02)
	{
		player.pcomm = player.comm;
		player.comm = int(player.comm) & 12;
	}
	else
	{
		player.comm = int(player.comm) & ~bit;
	}
}

void setDroneArrowControl(Simulation *sim, int bit, bool pressed)
{
	if (bit == 0x02)
		sim->SetFpvDroneControl(FPV_DRONE_RIGHT, pressed);
	else if (bit == 0x01)
		sim->SetFpvDroneControl(FPV_DRONE_LEFT, pressed);
	else if (bit == 0x04)
		sim->SetFpvDroneControl(FPV_DRONE_UP, pressed);
	else if (bit == 0x08)
		sim->SetFpvDroneControl(FPV_DRONE_DOWN, pressed);
}

void setArrowControl(Simulation *sim, int key, bool pressed)
{
	int bit = arrowCommandBit(key);
	if (!bit)
		return;

	if (!pressed)
	{
		clearArrowCommand(sim->player, bit);
		clearArrowCommand(sim->human, bit);
		if (sim->fpvDrone.active)
			setDroneArrowControl(sim, bit, false);
		return;
	}

	int control = activeArrowControl(sim);
	if (control == CONTROL_DRON)
	{
		setDroneArrowControl(sim, bit, true);
		return;
	}

	playerst *player = control == CONTROL_HMAN ? &sim->human : &sim->player;
	if (control == CONTROL_HMAN && bit == 0x08)
		return;
	if ((bit == 0x04 || bit == 0x08) && (int(player->comm) & bit))
		return;
	player->comm = int(player->comm) | bit;
}
}
...
void GameController::TransformPlaceSave(Mat2<int> transform, float rotationDegrees, Vec2<int> nudge)
{
	gameModel->TransformPlaceSave(transform, rotationDegrees, nudge);
}
...
void GameController::SetEditLayer(int layer)
{
	gameModel->SetEditLayer(layer);
}

void GameController::CycleEditLayer(int direction)
{
	gameModel->CycleEditLayer(direction);
}

int GameController::GetEditLayer() const
{
	return gameModel->GetEditLayer();
}
