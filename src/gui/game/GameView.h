...
private:
	...
	static constexpr int editLayerButtonCount = 4;
	std::array<ui::Button*, editLayerButtonCount> editLayerButtons{};
	void UpdateEditLayerButtons(int activeLayer);
	void UpdateEditLayerButtonFade();
	...
	float placeSaveRotationDegrees = 0.0f;
	void TranslateSave(Vec2<int> addToTranslate);
	void TransformSave(Mat2<int> mulToTransform);
	void RotateSave(float degrees);
	void PromptRotateSave();
