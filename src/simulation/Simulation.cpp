...
void Simulation::Load(const GameSave *save, bool includePressure, Vec2<int> blockP) // block coordinates
{
	...
	auto removeExistingParticles = [this, pasteArea, &existingParticles, &existingParticleIndices](Vec2<int> p) {
		...
		if (existingParticleIndices.Size().OriginRect().Contains(rp))
		{
			...
			{
				if (editLayersEnabled && parts[it->id].editLayer != editLayer)
				{
					continue;
				}
				kill_part(it->id);
	...
	for (int n = 0; n < NPART && n < save->particlesCount; n++)
	{
		...
		if ((player.spwn == 1 && tempPart.type==PT_STKM) || (player2.spwn == 1 && tempPart.type==PT_STKM2) || (human.spwn == 1 && tempPart.type==PT_HMAN))
		{
			continue;
		}
		...
		tempPart.editLayer = editLayersEnabled ? editLayer : 0;
		...
		case PT_HMAN:
			human = {};
			human.legs[0] = parts[i].x - 1.0f;
			human.legs[1] = parts[i].y + 12.0f;
			human.legs[4] = parts[i].x - 4.0f;
			human.legs[5] = parts[i].y + 22.0f;
			human.legs[8] = parts[i].x + 1.0f;
			human.legs[9] = parts[i].y + 12.0f;
			human.legs[12] = parts[i].x + 4.0f;
			human.legs[13] = parts[i].y + 22.0f;
			human.spwn = 1;
			human.elem = PT_NONE;
			human.spawnID = -1;
			humanPose = {};
			parts[i].ctype = PT_NONE;
			activeControllable = CONTROL_HMAN;
			break;
		case PT_SPAWN:
			player.spawnID = i;
			break;
	...
	if (editLayersEnabled)
	{
		RebuildMapsForEditLayer();
	}
	else
	{
		RecalcFreeParticles(false);
...
void Simulation::clear_sim(void)
{
	...
	editLayersEnabled = false;
	editLayer = 0;
	artilleryAiming = false;
	artilleryShells.clear();
	fpvDrone = {};
	humanDebris.clear();
	activeControllable = CONTROL_STKM;
...
int Simulation::try_move(int i, int x, int y, int nx, int ny)
{
	...
	if (!e) //if no movement
	{
		...
		{
			//@ WOOD -> SAWD
			float velSq = parts[i].vx * parts[i].vx + parts[i].vy * parts[i].vy;
			if (velSq > 25.0f)
...
int Simulation::create_part(int p, int x, int y, int t, int v)
{
	...
	if (p == -2)
	{
		auto pmapEditable = pmap[y][x] && MapMatchesEditLayer(pmap[y][x]);
		auto photonEditable = photons[y][x] && MapMatchesEditLayer(photons[y][x]);
		if (pmapEditable)
		{
			...
			return -1;
		else if (photonEditable && (elements[t].Properties & TYPE_ENERGY))
			return -1;
	}
	...
	parts[i].x = (float)x;
	parts[i].y = (float)y;
	parts[i].editLayer = editLayersEnabled && p != -3 ? editLayer : 0;

	//and finally set the pmap/photon maps to the newly created particle
	if (elements[t].Properties & TYPE_ENERGY)
		photons[y][x] = PMAP(i, t);
	else if (t!=PT_STKM && t!=PT_STKM2 && t!=PT_FIGH && t!=PT_HMAN)
		pmap[y][x] = PMAP(i, t);
...
void Simulation::delete_part(int x, int y)//calls kill_part with the particle located at x,y
{
	...
	if (editLayersEnabled)
	{
		auto layerPart = FindEditLayerParticle(x, y);
		if (layerPart == -1)
		{
			return;
		}
	...
	for (auto i = start; i < end && i < parts.active; i++)
	{
		...
		if (bmap[y/CELL][x/CELL] &&
		   (bmap[y/CELL][x/CELL]==WL_WALL ||
		    ...
		    (bmap[y/CELL][x/CELL]==WL_EWALL && !emap[y/CELL][x/CELL])) && (t!=PT_STKM) && (t!=PT_STKM2) && (t!=PT_FIGH) && (t!=PT_HMAN))
		{
			kill_part(i);
			continue;
		}
...
void SimulationImpl::MovementPhase(int i, Neighbourhood neighbourhood)
{
	...
	if (t==PT_STKM || t==PT_STKM2 || t==PT_FIGH || t==PT_HMAN)
	{
	...
	for (int i = 0; i < parts.active; i++)
	{
		...
		if (x>=0 && y>=0 && x<XRES && y<YRES)
		{
			...
			else
			{
				...
				if (t!=PT_THDR && t!=PT_EMBR && t!=PT_FIGH && t!=PT_HMAN && t!=PT_PLSM)
					pmap_count[y][x]++;
			}
			inBounds = true;
		}
...
void Simulation::SetEditLayer(int newEditLayer)
{
	editLayersEnabled = true;
	editLayer = (newEditLayer % EditLayerCount + EditLayerCount) % EditLayerCount;
	RebuildMapsForEditLayer();
}

int Simulation::GetEditLayer() const
{
	return editLayer;
}

bool Simulation::GetEditLayersEnabled() const
{
	return editLayersEnabled;
}

void Simulation::FlattenEditLayers()
{
	if (!editLayersEnabled)
	{
		return;
...
bool Simulation::MapMatchesEditLayer(int mapValue) const
{
	return !editLayersEnabled || (mapValue && parts[ID(mapValue)].editLayer == editLayer);
}

int Simulation::FindEditLayerParticle(int x, int y) const
{
	int normal = -1;
	int energy = -1;
	...
	for (int i = 0; i < parts.active; ++i)
	{
		...
		if (!part.type || part.editLayer != editLayer)
		{
			continue;
		}
		if (int(part.x + 0.5f) != x || int(part.y + 0.5f) != y)
		{
			continue;
		}
	...
	return energy != -1 ? energy : normal;
}

void Simulation::RebuildMapsForEditLayer()
{
	memset(pmap, 0, sizeof(pmap));
	memset(pmap_count, 0, sizeof(pmap_count));
	memset(photons, 0, sizeof(photons));
	...
	for (int i = 0; i < parts.active; ++i)
	{
		...
		if (x >= 0 && y >= 0 && x < XRES && y < YRES)
		{
			if (elements[t].Properties & TYPE_ENERGY)
			{
				if (!photons[y][x] || part.editLayer == editLayer)
				{
					photons[y][x] = PMAP(i, t);
				}
			}
			...
			{
				if (part.editLayer == editLayer ||
					!pmap[y][x] ||
					(!MapMatchesEditLayer(pmap[y][x]) && t != PT_INVIS && t != PT_FILT))
				{
					pmap[y][x] = PMAP(i, t);
				}
				if (t != PT_THDR && t != PT_EMBR && t != PT_FIGH && t != PT_HMAN && t != PT_PLSM)
...
void Simulation::BeginArtilleryAim(Vec2<int> start)
{
	artilleryAiming = true;
	artilleryAimStart = start;
	artilleryAimEnd = start;
}

void Simulation::UpdateArtilleryAim(Vec2<int> end)
{
	if (artilleryAiming)
	{
		artilleryAimEnd = end;
...
void Simulation::FireArtilleryShell(Vec2<int> start, Vec2<int> end, int blast)
{
	auto delta = end - start;
	auto distance = std::sqrt(float(delta.X * delta.X + delta.Y * delta.Y));
	artilleryAiming = false;
	if (distance < 2.0f || artilleryShells.size() >= 64)
	{
		return;
	}

	auto speed = std::clamp(distance * 0.13f, 4.0f, 28.0f);
	ArtilleryShell shell;
	shell.x = float(start.X);
	shell.y = float(start.Y);
	shell.vx = float(delta.X) / distance * speed;
	shell.vy = float(delta.Y) / distance * speed;
	shell.life = 320;
	shell.blast = std::clamp(blast, 1, 12);
	artilleryShells.push_back(shell);
}

void Simulation::BurstHuman(int i, Vec2<int> origin, int blast)
{
	if (i < 0 || i >= parts.active || parts[i].type != PT_HMAN)
		return;

	auto body = parts[i];
	auto limbState = human;
	auto armState = humanPose;
	auto blastScale = std::clamp(float(blast) * 0.42f, 1.0f, 5.5f);
	auto pressureVelocity = [this](Vec2<float> pos) {
		auto cellX = std::clamp(int(pos.X) / CELL, 0, XRES / CELL - 1);
		auto cellY = std::clamp(int(pos.Y) / CELL, 0, YRES / CELL - 1);
		auto samplePressure = [this](int x, int y) {
			x = std::clamp(x, 0, XRES / CELL - 1);
			y = std::clamp(y, 0, YRES / CELL - 1);
			return pv[y][x];
		};
		auto airX = vx[cellY][cellX] * 0.12f;
		auto airY = vy[cellY][cellX] * 0.12f;
		auto pressureX = (samplePressure(cellX - 1, cellY) - samplePressure(cellX + 1, cellY)) * 0.24f;
		auto pressureY = (samplePressure(cellX, cellY - 1) - samplePressure(cellX, cellY + 1)) * 0.24f;
		return Vec2<float>(airX + pressureX, airY + pressureY);
	};
	auto baseVelocity = Vec2<float>(body.vx, body.vy);
	auto velocityAway = [this, origin, blastScale, baseVelocity, pressureVelocity](Vec2<float> pos, float extra) {
		float dx = pos.X - float(origin.X);
		float dy = pos.Y - float(origin.Y);
		float dist = std::sqrt(std::max(dx * dx + dy * dy, 1.0f));
		auto jitterX = float(rng.between(-100, 100)) * 0.018f;
		auto jitterY = float(rng.between(-100, 100)) * 0.018f;
		return baseVelocity + pressureVelocity(pos) + Vec2<float>(dx / dist * (blastScale + extra) + jitterX, dy / dist * (blastScale + extra) + jitterY - 0.7f);
	};
	auto emit = [this, &body](int type, Vec2<float> pos, Vec2<float> vel, unsigned int colour, int life) {
		auto x = int(pos.X + 0.5f);
		auto y = int(pos.Y + 0.5f);
		if (!InBounds(x, y))
			return;
		auto np = create_part(-3, x, y, type);
		if (np < 0)
			return;
		parts[np].vx = vel.X;
		parts[np].vy = vel.Y;
		parts[np].temp = body.temp;
		parts[np].dcolour = 0xFF000000 | (colour & 0xFFFFFF);
		if (type == PT_EMBR)
		{
			parts[np].ctype = colour & 0xFFFFFF;
			parts[np].tmp = 0;
		}
		if (life > 0)
			parts[np].life = life;
	...
	auto skinColour = 0xFFD0A0u;
	auto limbColour = 0xF6F6F6u;
	auto bloodColour = 0xB00000u;
	auto shoulderLeft = Vec2<float>(body.x - 1.7f, body.y + 5.0f);
	auto shoulderRight = Vec2<float>(body.x + 1.7f, body.y + 5.0f);
	auto legRoot = Vec2<float>(body.x, body.y + 12.0f);
	auto leftKnee = Vec2<float>(limbState.legs[0], limbState.legs[1]);
	auto leftFoot = Vec2<float>(limbState.legs[4], limbState.legs[5]);
	auto rightKnee = Vec2<float>(limbState.legs[8], limbState.legs[9]);
	auto rightFoot = Vec2<float>(limbState.legs[12], limbState.legs[13]);
	auto leftElbow = armState.ready ? Vec2<float>(armState.arms[0], armState.arms[1]) : Vec2<float>(body.x - 4.0f, body.y + 9.0f);
	auto leftHand = armState.ready ? Vec2<float>(armState.arms[2], armState.arms[3]) : Vec2<float>(body.x - 5.0f, body.y + 13.0f);
	auto rightElbow = armState.ready ? Vec2<float>(armState.arms[4], armState.arms[5]) : Vec2<float>(body.x + 4.0f, body.y + 9.0f);
	auto rightHand = armState.ready ? Vec2<float>(armState.arms[6], armState.arms[7]) : Vec2<float>(body.x + 5.0f, body.y + 13.0f);
	auto pushDebris = [this](HumanDebris debris) {
		if (humanDebris.size() >= 96)
			humanDebris.erase(humanDebris.begin());
		humanDebris.push_back(debris);
	};
	auto spawnSegment = [this, &pushDebris, &velocityAway](Vec2<float> from, Vec2<float> to, unsigned int colour, float extra) {
		auto delta = to - from;
		auto length = std::sqrt(delta.X * delta.X + delta.Y * delta.Y);
		if (length < 0.5f)
			return;
		auto centre = (from + to) * 0.5f;
		auto velocity = velocityAway(centre, extra);
		HumanDebris debris;
		debris.x = centre.X;
		debris.y = centre.Y;
		debris.vx = velocity.X;
		debris.vy = velocity.Y;
		debris.angle = std::atan2(delta.Y, delta.X);
		debris.spin = float(rng.between(-100, 100)) * 0.006f + (velocity.X * delta.Y - velocity.Y * delta.X) * 0.0015f;
		debris.halfLength = std::max(1.4f, length * 0.5f);
		debris.colour = colour;
		debris.life = rng.between(210, 300);
		debris.kind = 0;
		pushDebris(debris);
	};
	auto spawnHead = [this, &pushDebris, &velocityAway, skinColour](Vec2<float> centre) {
		auto velocity = velocityAway(centre, 1.3f);
		HumanDebris debris;
		debris.x = centre.X;
		debris.y = centre.Y;
		debris.vx = velocity.X;
		debris.vy = velocity.Y;
		debris.angle = float(rng.between(0, 6283)) * 0.001f;
		debris.spin = float(rng.between(-100, 100)) * 0.008f;
		debris.halfLength = 3.0f;
		debris.colour = skinColour;
		debris.life = rng.between(230, 320);
		debris.kind = 1;
		pushDebris(debris);
	};

	spawnHead(Vec2<float>(body.x, body.y));
	spawnSegment(Vec2<float>(body.x, body.y + 4.0f), legRoot, limbColour, 1.0f);
	spawnSegment(shoulderLeft, leftElbow, limbColour, 1.2f);
	spawnSegment(leftElbow, leftHand, limbColour, 1.3f);
	spawnSegment(shoulderRight, rightElbow, limbColour, 1.2f);
	spawnSegment(rightElbow, rightHand, limbColour, 1.3f);
	spawnSegment(legRoot, leftKnee, limbColour, 1.3f);
	spawnSegment(leftKnee, leftFoot, limbColour, 1.4f);
	spawnSegment(legRoot, rightKnee, limbColour, 1.3f);
	spawnSegment(rightKnee, rightFoot, limbColour, 1.4f);
	for (auto n = 0; n < 22; n++)
	{
		auto point = Vec2<float>(body.x + float(rng.between(-5, 5)), body.y + float(rng.between(-2, 10)));
		emit(PT_EMBR, point, velocityAway(point, float(rng.between(0, 160)) * 0.01f), bloodColour, rng.between(35, 85));
	}

	kill_part(i);
}

void Simulation::UpdateHumanDebris()
{
	if (humanDebris.empty())
		return;

	std::vector<HumanDebris> liveDebris;
	liveDebris.reserve(humanDebris.size());
	auto const &elements = SimulationData::CRef().elements;
	auto blocked = [this, &elements](int x, int y) {
		if (!InBounds(x, y) || IsWallBlocking(x, y, PT_DUST))
			return true;
		auto r = pmap[y][x];
		if (r)
		{
			auto rt = TYP(r);
			if (rt != PT_EMBR && !(elements[rt].Properties & TYPE_GAS))
				return true;
		}
		return photons[y][x] != 0;
	};
	auto flowAt = [this](Vec2<float> pos) {
		auto cellX = std::clamp(int(pos.X) / CELL, 0, XRES / CELL - 1);
		auto cellY = std::clamp(int(pos.Y) / CELL, 0, YRES / CELL - 1);
		auto samplePressure = [this](int x, int y) {
			x = std::clamp(x, 0, XRES / CELL - 1);
		...
		auto pressureX = (samplePressure(cellX - 1, cellY) - samplePressure(cellX + 1, cellY)) * 0.035f;
		auto pressureY = (samplePressure(cellX, cellY - 1) - samplePressure(cellX, cellY + 1)) * 0.035f;
		return Vec2<float>(vx[cellY][cellX] * 0.035f + pressureX, vy[cellY][cellX] * 0.035f + pressureY);
	};
	auto collides = [&blocked](HumanDebris const &debris, float x, float y) {
		auto dir = Vec2<float>{ std::cos(debris.angle), std::sin(debris.angle) };
		auto normal = Vec2<float>{ -dir.Y, dir.X };
		if (debris.kind == 1)
		{
			auto radius = std::max(2.0f, debris.halfLength);
			Vec2<float> points[5] = {
				Vec2<float>(x, y),
				Vec2<float>(x + radius, y),
				Vec2<float>(x - radius, y),
				Vec2<float>(x, y + radius),
				Vec2<float>(x, y - radius),
			};
			for (auto point : points)
			{
				if (blocked(int(point.X + 0.5f), int(point.Y + 0.5f)))
					return true;
			}
			return false;
		}

		for (auto along : { -debris.halfLength, 0.0f, debris.halfLength })
		{
			for (auto side : { -1.0f, 0.0f, 1.0f })
			{
				auto point = Vec2<float>(x, y) + dir * along + normal * side;
				if (blocked(int(point.X + 0.5f), int(point.Y + 0.5f)))
					return true;
			}
		}
	...
	for (auto debris : humanDebris)
	{
		if (--debris.life <= 0)
			continue;

		auto flow = flowAt(Vec2<float>(debris.x, debris.y));
		debris.vx += flow.X;
		debris.vy += flow.Y + 0.09f;
		debris.vx *= 0.997f;
		debris.vy *= 0.997f;
		debris.spin *= 0.992f;
		debris.angle += debris.spin;

		auto steps = std::max(1, int(std::ceil(std::max(std::abs(debris.vx), std::abs(debris.vy)))));
		auto stepX = debris.vx / float(steps);
		auto stepY = debris.vy / float(steps);
		for (auto step = 0; step < steps; ++step)
		{
			debris.x += stepX;
			if (collides(debris, debris.x, debris.y))
			{
				debris.x -= stepX;
				debris.vx *= -0.34f;
				debris.spin += stepY * 0.045f;
			}
			...
			{
				debris.y -= stepY;
				debris.spin += debris.vx * 0.04f;
				debris.vy *= -0.24f;
				debris.vx *= 0.78f;
				if (std::abs(debris.vy) < 0.16f)
					debris.vy = 0.0f;
			}
		}

		if (std::abs(debris.vx) < 0.03f && std::abs(debris.vy) < 0.03f)
		{
			debris.vx = 0.0f;
			debris.vy = 0.0f;
			debris.spin *= 0.78f;
		...
		if (debris.x < -16.0f || debris.y < -32.0f || debris.x > XRES + 16.0f || debris.y > YRES + 16.0f)
			continue;

		liveDebris.push_back(debris);
	}

	humanDebris = std::move(liveDebris);
}

void Simulation::ExplodeArtilleryShell(Vec2<int> position, int blast)
{
	...
	auto radius = std::clamp(6 + blast * 3, 8, 42);
	auto radiusSq = radius * radius;
	auto humanFragilityRadius = float(radius) + 12.0f;
	auto humanFragilityRadiusSq = humanFragilityRadius * humanFragilityRadius;
	for (auto pi = 0; pi < parts.active; ++pi)
	{
		if (parts[pi].type != PT_HMAN)
			continue;
		auto dx = parts[pi].x - float(position.X);
		auto dy = parts[pi].y - float(position.Y);
		if (dx * dx + dy * dy <= humanFragilityRadiusSq)
		{
			BurstHuman(pi, position, blast);
			break;
		}
	}
	for (auto dy = -radius; dy <= radius; ++dy)
	{
		for (auto dx = -radius; dx <= radius; ++dx)
		{
			...
			auto dist = std::sqrt(float(std::max(distSq, 1)));
			auto falloff = 1.0f - std::min(dist / float(radius), 1.0f);
			auto pressure = float(blast) * 0.42f * falloff;
			pv[y / CELL][x / CELL] = std::clamp(pv[y / CELL][x / CELL] + pressure, MIN_PRESSURE, MAX_PRESSURE);
			...
			r = pmap[y][x];
			auto rt = TYP(r);
			if (rt && !(elements[rt].Properties & TYPE_GAS) &&
				rt != PT_DMND && rt != PT_CLNE && rt != PT_PCLN && rt != PT_BCLN && rt != PT_VIBR)
			{
				if (rt == PT_HMAN)
				{
					BurstHuman(ID(r), position, blast);
					continue;
				}
				auto angle = std::atan2(float(dy), float(dx));
				parts[ID(r)].vx += std::cos(angle) * float(blast) * 0.7f * falloff;
				parts[ID(r)].vy += std::sin(angle) * float(blast) * 0.7f * falloff;
				if (elements[rt].HighPressureTransition > -1 && elements[rt].HighPressureTransition < PT_NUM)
				{
					part_change_type(ID(r), x, y, elements[rt].HighPressureTransition);
				}
				else if (dist < float(radius) * 0.55f || rng.chance(blast, 18))
				{
					kill_part(ID(r));
				}
			}

			auto pmapIsGas = r && (elements[rt].Properties & TYPE_GAS);
			auto blockedByNonGas = r && !pmapIsGas;
			if (!pmapIsGas && rng.chance(1, std::max(2, 15 - blast)) && (!blockedByNonGas || dist > float(radius) * 0.4f))
			{
				auto nb = create_part(-3, x, y, PT_EMBR);
				if (nb != -1)
				{
					parts[nb].tmp = 0;
					parts[nb].life = rng.between(10, 45);
					parts[nb].temp = MAX_TEMP;
					parts[nb].vx = float(dx) * 0.08f + float(rng.between(-15, 15)) * 0.1f;
					parts[nb].vy = float(dy) * 0.08f + float(rng.between(-15, 15)) * 0.1f;
				}
			}
		}
	}
...
void Simulation::UpdateArtilleryShells()
{
	if (artilleryShells.empty())
	{
		return;
	}

	std::vector<ArtilleryShell> liveShells;
	liveShells.reserve(artilleryShells.size());
	auto const &elements = SimulationData::CRef().elements;
	auto shellBlocked = [this, &elements](int x, int y) {
		if (!InBounds(x, y) || IsWallBlocking(x, y, PT_BOMB))
		{
			return true;
		}
		auto r = pmap[y][x];
		if (r && !(elements[TYP(r)].Properties & TYPE_GAS))
		{
			return true;
		}
		return photons[y][x] != 0;
	...
	for (auto shell : artilleryShells)
	{
		auto explode = false;
		auto explodePos = Vec2<int>{ int(shell.x + 0.5f), int(shell.y + 0.5f) };
		auto steps = std::max(1, int(std::ceil(std::max(std::abs(shell.vx), std::abs(shell.vy)))));
		auto stepVelocity = Vec2<float>{ shell.vx / float(steps), shell.vy / float(steps) };
		for (auto step = 0; step < steps; ++step)
		{
			shell.x += stepVelocity.X;
			shell.y += stepVelocity.Y;
			auto x = int(shell.x + 0.5f);
			auto y = int(shell.y + 0.5f);
			explodePos = { x, y };
			if (shellBlocked(x, y))
			{
				explode = true;
		...
		if (explode)
		{
			ExplodeArtilleryShell(explodePos, shell.blast);
		}
		else
		{
			liveShells.push_back(shell);
		}
	}

	artilleryShells = std::move(liveShells);
}

void Simulation::SpawnFpvDrone(Vec2<int> position, int blast)
{
	if (!InBounds(position.X, position.Y))
	{
		return;
	...
	fpvDrone.active = true;
	fpvDrone.x = float(position.X);
	fpvDrone.y = float(position.Y);
	fpvDrone.vx = 0.0f;
	fpvDrone.vy = 0.0f;
	fpvDrone.roll = 0.0f;
	fpvDrone.controls = 0;
	fpvDrone.blast = std::clamp(blast, 1, 12);
	activeControllable = CONTROL_DRON;
}

void Simulation::SetFpvDroneControl(int control, bool pressed)
{
	if (pressed)
	{
		fpvDrone.controls |= control;
	}
	else
	{
		fpvDrone.controls &= ~control;
...
void Simulation::UpdateFpvDrone()
{
	if (!fpvDrone.active)
	{
		return;
	...
	auto &drone = fpvDrone;
	auto steer = 0.0f;
	if (drone.controls & FPV_DRONE_LEFT)
	{
		drone.vx -= 0.08f;
		steer -= 1.0f;
	}
	if (drone.controls & FPV_DRONE_RIGHT)
	{
		drone.vx += 0.08f;
		steer += 1.0f;
	}
	if (drone.controls & FPV_DRONE_UP)
	{
		drone.vy -= 0.12f;
	}
	if (drone.controls & FPV_DRONE_DOWN)
	{
		drone.vy += 0.06f;
		drone.vx *= 0.995f;
	}
	...
	auto speed = std::sqrt(drone.vx * drone.vx + drone.vy * drone.vy);
	if (speed > 5.2f)
	{
		auto limit = 5.2f / speed;
		drone.vx *= limit;
		drone.vy *= limit;
	}

	auto targetRoll = std::clamp(steer * 0.42f + drone.vx * 0.035f, -0.58f, 0.58f);
	drone.roll += (targetRoll - drone.roll) * 0.16f;

	auto const &elements = SimulationData::CRef().elements;
	auto droneBlocked = [this, &elements](int x, int y) {
		if (!InBounds(x, y) || IsWallBlocking(x, y, PT_BOMB))
		{
			return true;
		}
	...
	auto collides = [&droneBlocked](int x, int y) {
		for (auto dy = -2; dy <= 2; ++dy)
		{
			for (auto dx = -2; dx <= 2; ++dx)
			{
				if (dx * dx + dy * dy <= 4 && droneBlocked(x + dx, y + dy))
				{
					return true;
				}
			}
	...
	auto steps = std::max(1, int(std::ceil(std::max(std::abs(drone.vx), std::abs(drone.vy)))));
	auto stepVelocity = Vec2<float>{ drone.vx / float(steps), drone.vy / float(steps) };
	for (auto step = 0; step < steps; ++step)
	{
		drone.x += stepVelocity.X;
		drone.y += stepVelocity.Y;
		auto x = int(drone.x + 0.5f);
		auto y = int(drone.y + 0.5f);
		if (collides(x, y))
		{
			drone.active = false;
			ExplodeArtilleryShell({ x, y }, drone.blast);
			return;
		}
	}
	...
	if (InBounds(x, y))
	{
		auto right = Vec2<float>{ std::cos(drone.roll), std::sin(drone.roll) };
		auto down = Vec2<float>{ -right.Y, right.X };
		auto up = down * -1.0f;
		auto centre = Vec2<float>{ drone.x, drone.y };
		auto addRotorDraft = [this, &right, &down](Vec2<float> rotor, float strength) {
			for (auto depth = 0; depth < 5; ++depth)
			{
				for (auto spread = -2; spread <= 2; ++spread)
				{
					auto falloff = (5.0f - float(depth)) / 5.0f * (1.0f - std::abs(float(spread)) * 0.18f);
					auto pos = rotor + down * float(depth * CELL + 2) + right * float(spread * CELL / 2);
					auto px = int(pos.X + 0.5f);
					auto py = int(pos.Y + 0.5f);
					if (!InBounds(px, py))
					{
						continue;
					}
					auto cellX = px / CELL;
					auto cellY = py / CELL;
					vx[cellY][cellX] += down.X * strength * falloff;
					vy[cellY][cellX] += down.Y * strength * falloff;
					pv[cellY][cellX] = std::clamp(pv[cellY][cellX] + 0.015f * falloff, MIN_PRESSURE, MAX_PRESSURE);
				}
			}
		};

		auto draftStrength = (drone.controls & FPV_DRONE_UP) ? 0.22f : 0.11f;
		addRotorDraft(centre - right * 3.8f + up * 3.0f, draftStrength);
		addRotorDraft(centre + right * 3.8f + up * 3.0f, draftStrength);
	}
}
...
void Simulation::BeforeSim(bool willUpdate)
{
	...
	{
		if (editLayersEnabled && !willUpdate)
		{
			RebuildMapsForEditLayer();
		}
		else
