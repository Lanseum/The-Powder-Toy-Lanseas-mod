...
constexpr int FPV_DRONE_LEFT  = 0x01;
constexpr int FPV_DRONE_RIGHT = 0x02;
constexpr int FPV_DRONE_UP    = 0x04;
constexpr int FPV_DRONE_DOWN  = 0x08;

constexpr int CONTROL_STKM = 0;
constexpr int CONTROL_HMAN = 1;
constexpr int CONTROL_DRON = 2;
...
struct RenderableSimulation
{
	...
	static constexpr int EditLayerCount = 4;
	bool editLayersEnabled = false;
	int editLayer = 0;
	bool artilleryAiming = false;
	Vec2<int> artilleryAimStart = Vec2<int>::Zero;
	Vec2<int> artilleryAimEnd = Vec2<int>::Zero;
	std::vector<ArtilleryShell> artilleryShells;
	FpvDrone fpvDrone;
	std::vector<HumanDebris> humanDebris;
	int activeControllable = CONTROL_STKM;
...
public:
	...
	void SetEditLayer(int newEditLayer);
	int GetEditLayer() const;
	bool GetEditLayersEnabled() const;
	void FlattenEditLayers();
	void RebuildMapsForEditLayer();
	bool MapMatchesEditLayer(int mapValue) const;
	int FindEditLayerParticle(int x, int y) const;
	void BeginArtilleryAim(Vec2<int> start);
	void UpdateArtilleryAim(Vec2<int> end);
	void FireArtilleryShell(Vec2<int> start, Vec2<int> end, int blast);
	void UpdateArtilleryShells();
	void ExplodeArtilleryShell(Vec2<int> position, int blast);
	void BurstHuman(int i, Vec2<int> origin, int blast);
	void UpdateHumanDebris();
	void SpawnFpvDrone(Vec2<int> position, int blast);
	void SetFpvDroneControl(int control, bool pressed);
	void UpdateFpvDrone();
