...
void Simulation::Restore(const Snapshot &snap)
{
	...
	auto fighterCount = std::min<size_t>(MAX_FIGHTERS, snap.stickmen.size());
	std::copy(snap.stickmen.begin(), snap.stickmen.begin() + fighterCount, &fighters[0]);
	player = {};
	player.spawnID = -1;
	player2 = {};
	player2.spawnID = -1;
	...
	human.spawnID = -1;
	humanPose = {};
	auto extraStickmen = snap.stickmen.size() > MAX_FIGHTERS ? snap.stickmen.size() - MAX_FIGHTERS : 0;
	if (extraStickmen >= 3)
	{
		human   = snap.stickmen[MAX_FIGHTERS];
		player2 = snap.stickmen[MAX_FIGHTERS + 1];
		player  = snap.stickmen[MAX_FIGHTERS + 2];
	}
	else if (extraStickmen >= 2)
	{
		player2 = snap.stickmen[MAX_FIGHTERS];
		player  = snap.stickmen[MAX_FIGHTERS + 1];
	}
	{
...
int Simulation::CreatePartFlags(int p, int x, int y, int c, int flags)
{
	...
	if (flags & REPLACE_MODE)
	{
		...
		if ((!replaceModeSelected && (photonEditable || pmapEditable)) ||
			(!photonEditable && pmapEditable && TYP(pmap[y][x]) == replaceModeSelected) ||
			(photonEditable && TYP(photons[y][x]) == replaceModeSelected))
		{
			auto target = photonEditable ? photons[y][x] : pmap[y][x];
			if (c)
				create_part(ID(target), x, y, TYP(c), ID(c));
			else
				delete_part(x, y);
		}
		return 0;
